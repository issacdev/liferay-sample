...
public class ConfirmEditSampleListener implements IInit, ClickListener {
	...
	public void buttonClick(ClickEvent event) {
		IEnquiryView enquiryView = (IEnquiryView) view.getView("EditSampleView");
		...
		if (enquiryView.validate()) {
			...
			if (newData.get(SampleConstant.VALUE) != null) {
				String changeValue = (String) newData.get(SampleConstant.VALUE);
				row.put(SampleConstant.VALUE, changeValue);
			}

			try {
				Boolean result = ((SampleController) control).editSample(row);
				if (result) {
					control.showNotification(message.getString("sample.message.submit.success"), 8000);
					((Window) view).removeWindow(event.getButton().getWindow());
					((SampleController) control).refreshTable();
				} else {
					control.showNotification(message.getString("sample.message.submit.fail"), 8000);
				}
			} catch (Exception e) {
